...
public class ReDianFragment extends MyFragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		if (v == null) {
			...
			v = inflater.inflate(R.layout.fragment_redian, container, false);
			isReady = true;
			delayLoad();
	...
	public void handleResponse(String response) {
		...
		try {
			...
			for (int i = 0; i < str_biaoti.length; i++) {
				zixun = new ZiXun(str_biaoti[i], str_laiyuan[i], str_yuedu[i],
						str_shijian[i], str_tupian[i].replace("\\", ""),
						str_neirong[i], str_lianjie[i].replace("\\", ""));
