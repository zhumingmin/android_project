...
### 12.8项目开发日志
农业资讯存在的问题：  
1.点击不同的类别时不能加载对应的内容（上传类别再从数据库获取最新的一条记录是否靠谱？）  
2.图片无法显示  
3.点击单个item无法将数据传递给详情activity  
4.fragment的操作冲突问题

### 12.9项目开发日志
1.如何解决fragment的预加载问题？  
2.如何解决网络加载的数据缓存问题？

### 12.12项目开发日志
1.农业资讯详情可以看到了  
2.图片显示的问题（listview中每个item的图片显示）获取后台的图片网络地址→使用volley加载显示在imageview中  
3.对应主题的资讯不对应（点击tab切换时资讯出现错位的现象）

### 12.13项目开发日志
1.fragment+viewpager的预加载问题  
2.图片的显示加载问题  
3.相应类别下的资讯对应问题  
4.点击搜索后不能及时加载更新

### 12.18项目开发日志
1.账号体系的研究（什么情况下需要每次都登录，什么时候需要）  
2.查询一下功能的完善  
3.关于缓存的研究  
4.关于适配低版本手机的闪退问题  
5.无法连接服务器的状态下怎么使用APP

一号货的可以参考的地方：  
1.我的设置  
2.进入APP先不用登录，办理业务时才需要登录
